...
Referring to the previous section, when one look at the Number of Transactions per Block and Network Difficulty graph over the years, one can make the observation that the difficulty has significantly increased over time.
...
### Orphan blocks
On the topic of block generation, another interesting question pertains to the case of forks when two or more blocks are generated within seconds of each other.
...
If the fork is of equal length, the miners generate blocks onto whichever block that they receive first. Consequently, one of the chain would be longer (with 
