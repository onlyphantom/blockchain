...
4. Timestamp: The time the block was created.
...
### Timestamp
The Unix timestamp as seconds since 1970-01-01T00:00:00 (UTC). 
...
### Target
The target is a 256-bit (extremely large) number; the lower the number is the harder it is to find a hash that is below the target. This is because the SHA-256(SHA256()) function of a block header
...
### Nonce
An arbitrary number miners change in their attempt to produce a hash less than or equal to the target. It starts at 0 and increment by 1 each time a hash is generated.


What should happen if more than one 
