...
An implementation detail here is that this concatenation is performed on the byte sequences. 

While we represent the byte sequence in hexadecimal characters (`0x`), we perform the concatenation operation on the two byte sequences
